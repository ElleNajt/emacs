...
(defun new-python-project (project-name &optional base-dir on-complete)
  "Create a new Python project folder with a specified structure.
PROJECT-NAME is the name of the project.
BASE-DIR is the parent directory (defaults to current dired directory).
ON-COMPLETE is an optional callback called with PROJECT-DIR when setup finishes."
  (interactive "sProject name: ")
  (let* ((current-dir (or base-dir
                          (and (derived-mode-p 'dired-mode) (dired-current-directory))
                          default-directory))
         (project-dir (concat (file-name-as-directory current-dir) project-name "/"))
         (defaults-dir (concat doom-user-dir "python/project_defaults/"))
         (current-datetime (format-time-string "%Y-%m-%d %H:%M:%S"))
         (log-buffer-name (format "*new-python-project: %s*" project-name)))

    ;; Create log buffer for this project setup
    (with-current-buffer (get-buffer-create log-buffer-name)
      (erase-buffer)
      (insert (format "=== Creating Python project: %s ===\n" project-name))
    ...
    (cl-labels
        ((log-msg (msg)
           ...
           (message "%s: %s" project-name msg))

         (run-step (name command next-step)
           (log-msg (format "Starting: %s" name))
           (let ((output-buffer (generate-new-buffer (format " *%s-output*" name))))
             (make-process
              :name name
              :command (list "sh" "-c" (format "cd %s && %s 2>&1"
              ...
              (lambda (proc event)
                (let ((exit-code (process-exit-status proc))
                  ...
                  (if (= exit-code 0)
                      (progn
                        (log-msg (format "Completed: %s" name))
                        (when next-step (funcall next-step)))
                    (log-msg (format "FAILED: %s (exit code %d)" name exit-code))
                    (log-msg "Setup aborted. Check log buffer for details.")
                    (pop-to-buffer log-buffer-name))))))))

      (make-directory project-dir t)
      (log-msg "Created project directory")

      ;; Copy everything from defaults directory except .direnv
      (dolist (item (directory-files defaults-dir nil "^[^.]\\|^\\."))
        (unless (member item '("." ".." ".direnv" ".venv"))
          (let ((source (concat defaults-dir item))
                (dest (concat project-dir item)))
            (cond
             ((file-directory-p source)
              (copy-directory source dest nil t t)
              (log-msg (format "Copied directory: %s" item)))
             ...
             ((string= item "scratch.org")
              (write-region (with-temp-buffer
                              ...
                              (let ((content (buffer-string)))
                                ...
                                content))
                            nil dest)
              (log-msg (format "Copied and processed: %s" item)))

             (t
              (copy-file source dest)
              (log-msg (format "Copied file: %s" item)))))))

      ;; Run setup steps in sequence
      (run-step "uv-setup"
                "uv sync"
                (lambda ()
                  (run-step "direnv-allow"
                            "direnv allow ."
                            (lambda ()
                              (run-step "git-init"
                                        "git init && git branch -m main && yes n | bd init && git add . && git commit -m 'Initial commit'"
                                        (lambda ()
                                          (log-msg "=== Project setup complete! ===")
                                          (message "Python project '%s' created successfully!" project-name)
                                          (when on-complete
                                            (funcall on-complete project-dir)))))))))))
...
(defun tramp/claude-bot ()
  "Connect to claude-bot GCP VM via TRAMP.
Uses SSH config entry 'claude-bot' from ~/.ssh/config"
  (interactive)
...
(after! agent-shell 
  ...
  ;; Must inject into :environment-variables (not process-environment)
  ;; because the process is started lazily by acp--start-client.
