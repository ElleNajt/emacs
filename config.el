...
(after! magit
  ...
  (transient-define-suffix magit-commit-with-claude ()
    ...
    (let* ((diff (with-output-to-string
      ...
      (message "Generating commit message with Claude from %s..." default-directory)
      (make-process
       :name "claude-commit"
       :buffer buffer
       ...
       (lambda (process output)
         (message "Claude output chunk: %s" output)
       ...
       (lambda (process event)
         (let ((status (process-status process))
           ...
           (message "===== Claude process sentinel called =====")
           (message "Event: %s, Status: %s, Exit code: %s" event status exit-code)
           (message "Process buffer: %s, alive: %s" 
                    (process-buffer process)
                    (buffer-live-p (process-buffer process)))
           (when (buffer-live-p (process-buffer process))
             (message "Buffer contents length: %s" 
                      (with-current-buffer (process-buffer process)
                        (buffer-size))))
           (when (memq status '(exit signal))
             (if (and (eq status 'exit) (= exit-code 0))
                 (with-current-buffer (process-buffer process)
                   (let ((commit-msg (string-trim (buffer-string))))
                     (message "Claude response (length %s): %s" (length commit-msg) commit-msg)
                     (if (string-empty-p commit-msg)
                         (progn
                           (message "ERROR: Claude did not return a commit message")
                           (pop-to-buffer (process-buffer process)))
                       (message "About to call magit-commit-create...")
                       (magit-commit-create (list "-m" commit-msg))
                       (message "SUCCESS: Committed with Claude-generated message")
                       (kill-buffer (process-buffer process)))))
               (message "ERROR: Claude process failed with exit code %s" exit-code)
               (pop-to-buffer (process-buffer process)))))))))
  (transient-append-suffix
    #'magit-commit
    "c"
    (list "C" "Commit with Claude message" #'magit-commit-with-claude)))
